...
def Q_1(cursor, conn, execution_time):
    ...
    SELECT
        p.player_name,
        AVG(es.statsbomb_xg) AS avg_xg
    FROM
        events e
    JOIN
        event_shot es ON e.event_id = es.event_id
    JOIN
    ...
    JOIN
        seasons s ON m.season_id = s.season_id
    WHERE
        e.type_name = 'Shot'
        AND es.statsbomb_xg > 0
        AND c.name = 'La Liga'
        AND s.name = '2020/2021'
    GROUP BY
        p.player_name
    HAVING
        COUNT(es.event_id) > 0
...
def Q_3(cursor, conn, execution_time):
    ...
    SELECT
        p.player_name,
        COUNT(e.event_id) AS num_first_time_shots
    FROM
        events e
    JOIN
        event_shot es ON e.event_id = es.event_id AND es.first_time = TRUE
    JOIN
        players p ON e.player_id = p.player_id
    JOIN
        matches m ON e.match_id = m.match_id
    ...
    WHERE
        e.type_name = 'Shot'
        AND c.name = 'La Liga'
        AND s.name IN ('2020/2021', '2019/2020', '2018/2019')
    GROUP BY
        p.player_name
    HAVING
        COUNT(e.event_id) >= 1
...
def Q_5(cursor, conn, execution_time):
    ...
    SELECT
        p.player_name,
        COUNT(*) AS num_received_passes
    FROM
        event_pass ep
    JOIN
        players p ON ep.recipient_id = p.player_id
    JOIN
    ...
    WHERE
        ...
        AND s.name = '2003/2004'
    GROUP BY
        p.player_name
    HAVING
        COUNT(*) >= 1
    ORDER BY
        num_received_passes DESC;
    """
...
def Q_6(cursor, conn, execution_time):
    ...
    SELECT
        t.name AS team_name,
        COUNT(e.event_id) AS total_shots
    FROM
        teams t
    JOIN
        matches m ON t.team_id = m.home_team_id OR t.team_id = m.away_team_id
    ...
    JOIN
        seasons s ON m.season_id = s.season_id
    WHERE
        c.name = 'Premier League' AND
        s.name = '2003/2004' AND
        e.type_name = 'Shot'
    GROUP BY
        t.name
    HAVING
...
def Q_7(cursor, conn, execution_time):
    ...
    SELECT
        p.player_name,
        COUNT(ep.event_id) AS through_ball_count
    FROM
        players p
    JOIN
        events e ON p.player_id = e.player_id
    JOIN
        event_pass ep ON e.event_id = ep.event_id AND ep.technique_name = 'Through Ball'
    JOIN
        matches m ON e.match_id = m.match_id
    JOIN
        seasons s ON m.season_id = s.season_id
...
def Q_9(cursor, conn, execution_time):
    ...
    SELECT
        p.player_name,
        COUNT(ed.event_id) AS successful_dribbles
    FROM
        players p
    JOIN
        events e ON p.player_id = e.player_id
    JOIN
        event_dribble ed ON e.event_id = ed.event_id AND ed.outcome_name = 'Complete'
    JOIN
        matches m ON e.match_id = m.match_id
    JOIN
        competitions c ON m.competition_id = c.competition_id
    ...
    WHERE
        c.name = 'La Liga' AND
        (s.name = '2020/2021' OR s.name = '2019/2020' OR s.name = '2018/2019')
    GROUP BY
        p.player_name
    HAVING
        COUNT(ed.event_id) >= 1
...
def Q_10(cursor, conn, execution_time):
    ...
    SELECT
        p.player_name,
        COUNT(edp.event_id) AS dribbled_past_count
    FROM
        players p
    JOIN
        events e ON p.player_id = e.player_id
    JOIN
        event_dribbled_past edp ON e.event_id = edp.event_id
    JOIN
        matches m ON e.match_id = m.match_id
    JOIN
        competitions c ON m.competition_id = c.competition_id
    ...
    GROUP BY
        p.player_name
    HAVING
        COUNT(edp.event_id) >= 1
    ORDER BY
        dribbled_past_count ASC;
    """
